...
// Relegation is a same-context "covered without narrating" disposition (it
// excuses a deep internal from the lead's story), not a relationship to another
// context, so it never renders as a seam / leans-on or a workflow activation.
const RELEGATES_LINK_KEY = "relegates";
...
// The server's storyBuckets carries `why`/`when` as optional (tolerance for
// an older server, pre learning-plane WHY/WHEN reshape); default both to ""
// here so every other consumer can read all four fields unconditionally.
export function storyBucketsForCard(card: LibraryCatalogCard): PeekStory | null {
  if (card.storyBuckets != null) {
    return {
      how: card.storyBuckets.how,
      what: card.storyBuckets.what,
      when: card.storyBuckets.when ?? "",
      why: card.storyBuckets.why ?? "",
    };
  }
  if (card.story != null && card.story.length > 0) {
    return { how: "", what: card.story, when: "", why: "" };
  }
  if (card.synopsis != null && card.synopsis.length > 0) {
    return { how: "", what: card.synopsis, when: "", why: "" };
  }
  return null;
}
...
export function storyTextForOrdering(card: LibraryCatalogCard): string | undefined {
  ...
  return [buckets.how, buckets.what, buckets.why, buckets.when]
...
function typedLinks(
  ...
  for (const [key, targets] of Object.entries(card.links ?? {})) {
    if (key === CONTAINS_LINK_KEY || key === RELEGATES_LINK_KEY) {
      continue;
    }
...
export function peekHasContent(model: LibraryPeekModel): boolean {
  ...
  const hasStory =
    model.story != null &&
    (model.story.what.trim().length > 0 ||
      model.story.how.trim().length > 0 ||
      model.story.why.trim().length > 0 ||
      model.story.when.trim().length > 0);
  return (
    hasStory ||
