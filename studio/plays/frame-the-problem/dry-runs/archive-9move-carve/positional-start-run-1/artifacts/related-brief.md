...
**Circumstance:** A new customer has just created their Meridian account and lands on the main dashboard for the first time. The dashboard is empty because no data source is connected yet. The onboarding checklist that is supposed to guide them through setup is in the sidebar and requires scrolling to find. By the time they discover it — if they discover it — they have already concluded the product is complicated. The first impression is a blank screen with no obvious path forward.
...
**What it's not:** A request for a better checklist, an onboarding wizard, or a redesigned sidebar. The problem is that the customer's first encounter with Meridian is a blank screen that gives no signal about what to do, and the existing guidance is placed where they will not see it in time.

**Where it lands:** Onboarding checklist (hidden in sidebar, not surfaced on the empty dashboard state — confirmed by surface map: "The onboarding checklist is hidden in the sidebar and requires scrolling; it is not surfaced on the empty dashboard state customers see first") · Dashboard (empty until a data source is connected — confirmed by surface map: "shows pre-built charts from connected data. Empty until a data source is connected").
...
**Who:** Customer Success Manager (CSM). LEO is the speaker. Resolved against users file: "Runs implementation calls, onboards new accounts... Owns the customer relationship during the first 90 days."
...
**Circumstance:** SAM shipped the onboarding checklist two months ago. SAM pulled the activation numbers and found that 31% of new accounts complete the first three steps before day seven — unchanged from before the checklist shipped. The feature has had no measurable effect on the metric it was designed to move, and SAM does not know whether the cause is the product, the timing, or something else.
...
- Problem 1 → Problem 2: `subset-of` — Problem 1 (empty dashboard, no visible path) describes the customer's confusion at the front door; Problem 2 (stuck at the connector step) describes what happens when they find the door and try to walk through it. Both are stages of the same onboarding sequence, but they are distinct enough to attack separately: you could surface the checklist on the dashboard (solving Problem 1) and the customer would still hit the credential wall at the connector (Problem 2 untouched). The edge runs from 1 to 2 because the empty-dashboard confusion is a subset of the broader setup failure — it is the opening chapter, not a different book.

- Problem 2 → Problem 3: `suspected-root` — Problem 3 (LEO re-explaining setup on follow-up calls) exists because Problem 2 (customers stalling at the connector) leaves the product in a non-functional state with no saved progress. If the connector step did not stall customers and lose their progress, the follow-up call would not start from scratch. LEO's evidence makes the causal chain explicit: customers go dark after the connector call (Problem 2, evidence 3), then LEO re-explains the same setup on the follow-up (Problem 3, evidence 1). Problem 2 is the suspected root of Problem 3.

- Problem 1 → Problem 4: `subset-of` — Problem 4 (the checklist has not moved the activation metric) is partly explained by Problem 1 (customers do not see the checklist because it is hidden in the sidebar). If the checklist's intended audience never encounters it, the metric it was designed to move will not move. Problem 1 is a subset of Problem 4's diagnostic question — one candidate explanation for why the feature appears inert. This edge does not claim it is the only explanation; SAM explicitly does not know whether the cause is the product, timing, or something else (Problem 4, evidence 5).

- Problem 2 → Problem 4: `subset-of` — Even if customers find the checklist, Problem 2 (stalling at the connector step) would prevent them from completing the activation steps the metric tracks. The checklist leads customers into a step that blocks them, so the metric stays flat. This is a second candidate explanation sitting alongside the Problem 1 edge above; both feed into Problem 4's unanswered diagnostic question.

---

## Hunch

The connector step (Problem 2) looks like the root. Problems 1 and 3 both trace back to it: the empty dashboard (Problem 1) is the opening confusion, but customers who push past it still hit the connector wall — and when they do, the session dies and the follow-up call resets to zero (Problem 3). The checklist's flat metric (Problem 4) is consistent with both Problems 1 and 2, but SAM's own uncertainty about the cause means the diagnostic question in Problem 4 is not yet answered by evidence — this hunch does not claim to answer it. The hunch is limited to the undisputed structure: Problem 2 is the point where onboarding breaks, and Problem 3 is its direct consequence.
