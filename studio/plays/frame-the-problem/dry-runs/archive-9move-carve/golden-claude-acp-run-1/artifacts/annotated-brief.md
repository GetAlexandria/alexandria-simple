...
Prior bounce items (3): all resolved — `the next conversion session`, `before the next conversion session`, `what to convert next` each removed from brief text; sizing lexicon now clean.
