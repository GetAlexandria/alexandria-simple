...
export function runtimeEventStream(
  ...
  return Stream.asyncScoped<RuntimeStreamMessage, ViewerSubscriptionError>(
    (emit) => {
      const source = new EventSource(endpoint);
      const emitMessage =
        (eventName: RuntimeStreamMessage["event"]) =>
        (message: MessageEvent<string>) => {
      ...
      const emitFailure = (event: Event) => {
        ...
        emit(
          Effect.fail(
            Option.some(new ViewerSubscriptionError(endpoint, event)),
          ),
        );
      };
