...
> **HISTORICAL — 9-move era (2026-06-11).** This read-out records runs of the
> retired nine-move pipeline (locate → … → render → self_check → cold_reader)
> against the then-current answer key, using the Needle/Knot/Storm three-seam
> grading. The play has since been re-architected into the interactive "Riff"
> play (one `pre_fill` framing pass + a `review ⇄ revise` loop; one deliverable,
> `runtime/problem-framing.md`; the three-mark evidence scale `first-hand /
> assumed-hand-wavey / hypothetical`). The companion `answer-key.md` has been
> retuned to the Riff contract; this file is preserved as-is as the dated record
> of the 9-move runs and is NOT a grading target for the Riff play. The findings
> below (commitment vs near-miss; split-grade by clause; disputed-edge leak;
> noise-degrades-distinctness) carry forward as substance; their move/seam labels
> (locate, render, self_check, relate split) are 9-move artifacts.
