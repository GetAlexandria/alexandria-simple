# Alexandria: Story and Logic

> **Note:** This document was imported from LifeBuild issue #756. It uses LifeBuild-specific
> type names and historical issue terminology from the original design work as illustrative
> examples of the general pattern. Paths have been updated to reflect the plugin repo
> structure, but historical names such as "Wizard Engine" are preserved here where they are
> part of that imported design record.
...
**Audience:** Engineers working on Issue #753 (Alexandria Wizard Engine) and follow-on tooling.
...
The queue accumulates over time. During maintenance cycles, Conan reviews the
queue while assessing library health and prioritizing repairs. High-severity
items (blocked assembly or forced builder to guess on a critical dimension) get
addressed first.
...
**Who does it:** `ax lint` (metrics and regression checks), Conan (assessment,
diagnosis, prioritization, and review), Sam (fixes), human (priority
decisions).
...
Maintenance runs as one quality cycle with an assessment front-end and a repair
loop:

**Health Check (Job 8):** The periodic assessment front-end. `ax lint` provides the
quantitative backdrop, then Conan grades the standard maintenance sample,
checks high-blast-radius rationale surfaces, traces weak cards upstream through
cascade analysis, and synthesizes feedback/provenance signals into a repair
agenda.
...
**Diagnose (Job 3):** When Health Check identifies a problem cluster, Diagnose
traces it to root causes and calculates blast radius — how many other cards
would be affected by a fix. Blast radius informs prioritization: a fix to one
upstream card might resolve issues in twenty downstream cards.

**Recommend (Job 4):** Prioritizes fixes by cascade potential plus demand
signals from feedback and provenance. High blast radius + repeated builder pain
= fix first.
...
**Review (Job 5):** After Sam applies fixes and `ax lint` runs regression checks,
Conan re-grades and produces a delta report showing before/after scores.
...
### Phase 8: Federation — Beyond One Node

**What triggers it:** The organization operates multiple factories that produce different output types (code, prose, messages, media) and needs shared context across them.

**Who does it:** Human (organizational design decisions), Conan (cross-library cascade analysis), Sam (library construction at each altitude), Solomon (multi-altitude signal routing).

**The narrative:** Phases 1 through 7 describe a single factory library — one team, one output type, one knowledge graph. Phase 8 extends the system to organizations where product knowledge lives at multiple altitudes and multiple factories consume shared upstream context.

The altitude model has three levels:

- **Corporate context library:** Identity-layer knowledge shared across the entire organization — mission, values, strategic bets, competitive thesis, brand standards, cross-cutting governance. This is the conversation about "who are we and where are we going?"
- **Division context library:** Function-specific knowledge that inherits from corporate and adds domain strategy — product roadmap, technical architecture, channel strategy, audience models. This is the conversation about "how do we execute our part of the strategy?"
- **Factory context library:** Genus-specific tactical knowledge built using the full construction system (Phases 1-7). This is the conversation about "how do we print this well?" Each factory has its own type vocabulary determined by what its factory prints (Software, Prose, Communications, Media, etc.).

**Context inheritance operates downward with interpretation at each level.** A corporate strategic bet does not get copied into a factory library verbatim. The division creates a card that links back to the corporate card and says "here is what this bet means for our function." The factory creates a card that links to the division card and says "here is what this means for our daily production." Same upstream thesis, different instantiation at each altitude. The mechanism is cross-library wikilinks: a factory card references a corporate card, and the Strategy Cascade loop traces the blast radius across library boundaries when the upstream card changes.

**Signal flows upward as raw input without interpretation.** Marketplace signal — usage data, customer feedback, support tickets, revenue, competitive intelligence — flows straight to whichever altitude it is relevant to. A bug report goes to the software factory. A customer churn signal goes to divisional product strategy AND corporate strategy simultaneously. Signal does not get progressively interpreted on the way up; it arrives as raw data to be incorporated at the right altitude. The asymmetry is deliberate: context flows down with progressive interpretation because each level needs to translate strategy into local terms; signal flows up as raw input because the receiving level needs to draw its own conclusions.

**The WHY chain extends to cross-library scope.** Within a single factory library, the WHY dimension traces from a leaf card up through its rationale chain to Product Theses and Principles. In the federated model, that chain continues: a factory-level Product Thesis may link to a division-level strategy card, which links to a corporate-level strategic bet. Cascade analysis must traverse these cross-library links. A change to a corporate strategic bet triggers cascade analysis in every division and factory library that inherits from it. The blast radius is cross-factory, not just cross-card.

**The five-phase build sequence is mandatory at each node.** Each library in the federation — corporate, division, factory — goes through Configuration, Seeding, Assembly, Feedback, and Maintenance independently. The construction system (agents, initialize flow, tooling, quality loop) operates at every altitude; the type vocabulary and retrieval profiles change, but the graph structure, assembly pattern, and quality loop are invariant. Corporate context libraries use fewer types with heavier identity and governance layers. Factory context libraries use the full genus-specific type taxonomy.

**Documentation-as-conversation is the operating principle.** In the federated model, every card at every altitude is a participant in an ongoing organizational conversation, not a static reference document. Cards have explicit assumptions that can be challenged by incoming signal, validation criteria that define what would change them, and cascade links that propagate changes when they do change. The signal queue routes marketplace feedback to the right altitude. The feedback queue surfaces assembly gaps at each level. The strategy cascade propagates changes across library boundaries. This is the mechanism that prevents organizational balkanization — not a single monolithic knowledge base, but connected conversations at the right altitudes with explicit inheritance and signal flow.

**When to federate:** Federation is a Phase 4+ concern. A single team with one factory does not need it — the product library's identity layer serves as de facto corporate context. Federation becomes necessary when an organization has two or more factories that need to share context but are producing different outputs. The mandate: prove one complete node first. The entire build sequence (Phases 1-7) must be battle-tested at the single-factory level before expanding to multi-altitude deployment.
...
**Type-based retrieval profiles:** Each of the 21 card types has its own profile specifying what must be included, how many hops to traverse, dimension priority ordering, lateral scope, and anti-pattern requirements. Profiles are not suggestions — they are the rules. Missing a mandatory category is an assembly deficiency.
...
*Document produced: 2026-03-13. Source files: `docs/alexandria/README.md`, `skills/context-briefing/retrieval-profiles.md`, `skills/context-briefing/traversal.md`, `skills/context-briefing/protocol.md`, `skills/conan/rubrics.md`, `skills/conan/grade-computation.md`, `skills/sam/card-creation.md`, `skills/sam/self-check.md`, `skills/context-briefing/feedback-queue-schema.md`, `skills/context-briefing/provenance-schema.md`, `docs/alexandria/reference.md`.*
