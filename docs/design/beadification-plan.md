# Beadification Plan: Alexandria as Composable Product Knowledge Harness

> **Note:** This document was imported from LifeBuild issue #756. It was originally written
> for the LifeBuild monorepo (`packages/context-library/`). The architecture and concepts
> are applicable to this standalone plugin repo, but specific paths and package references
...
|---|---|---|
| #1 Initialize Engine | Phase 5 groundwork | `LibraryConfig` abstraction is the initialize flow's output format |
...
"context-library": {
  ...
  "env": { "CL_LIBRARY_PATH": "docs/alexandria" }
}
```
...
### 3b. Cross-reference conventions (`docs/alexandria/BEADS_INTEGRATION.md`)
...
### 4a. Update Conan's assembly procedure (`agents/conan.md`)
...
- `agents/conan.md` — MCP tool usage in assembly
- `skills/ax-brief/retrieval-profiles.md` — add YAML blocks
- `skills/ax-brief/traversal.md` — add programmatic section
- `skills/ax-brief/provenance-schema.md` — add `bead_id`
- `skills/conan/job-*.md` — use MCP tools
...
- `docs/alexandria/reference.md` — type taxonomy (2217 lines)
- `skills/ax-brief/retrieval-profiles.md` — assembly rules
- `skills/ax-brief/traversal.md` — graph navigation
