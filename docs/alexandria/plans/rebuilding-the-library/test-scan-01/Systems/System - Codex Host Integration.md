...
## WHAT
_Stub —_ The integration that connects Alexandria's agents to a coding tool (Codex / ACP, and a Claude adapter) so agents run where the code lives.

## WHERE
_Stub —_ `ax codex` launches Codex wired to local services; provides the host behind a [[Entity - Raven Connection]]; lets [[Agent - Raven]]/[[Agent - Damien]] act in the editor.
