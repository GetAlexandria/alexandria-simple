...
## WHAT
_Stub —_ The event-sourced backbone: appends events to a JSONL ledger and projects them into the derived project state every surface reads.

## WHERE
_Stub —_ Feeds [[Entity - Event Ledger]], the project-state projection (agents, playbook, raven, sources, cards), and the runtime HTTP server the viewer/CLI call.
