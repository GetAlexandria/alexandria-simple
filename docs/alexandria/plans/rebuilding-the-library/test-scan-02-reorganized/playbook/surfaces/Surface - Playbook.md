...
## WHAT
_Stub —_ The surface listing the product's [[Aggregate - Play]] definitions — the guided, agent-run workflows a user can launch.

## WHERE
_Stub —_ "Playbook" stone in [[Surface - Stone Top Bar]] (route `/playbook`). Launches a [[Aggregate - Play Run]] via the run launcher; plays are executed through [[System - Fabro Workflow Engine]] and observed in [[Surface - Play Maker's Studio]].
