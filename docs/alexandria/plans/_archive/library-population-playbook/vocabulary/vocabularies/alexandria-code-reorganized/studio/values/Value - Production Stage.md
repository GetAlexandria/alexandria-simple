...
## WHAT
_Stub —_ One of six columns a play passes through on its way to shipping: Empty → Sourced → Designed → Built → Proven → Live, each with a gate description. A pure enum/value-object — no lifecycle of its own.

## WHERE
_Stub —_ Columns of the [[Surface - Studio Board]] in [[Surface - Play Maker's Studio]]; a play advances one stage on a Director confirm.
